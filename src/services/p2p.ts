...
export class P2PManager {
  ...
  private async createPeerConnection(
    ...
    if (!iceServers) {
      ...
      this.sdk.notifyGame(WavedashEvents.P2P_CONNECTION_FAILED, {
        userId: remoteUserId,
  ...
  private setupDataChannelHandlers(
    ...
    channel.onopen = () => {
      ...
      if (this.isPeerReady(remoteUserId)) {
        const peer = this.currentConnection?.peers[remoteUserId];
        if (peer) {
          this.sdk.notifyGame(WavedashEvents.P2P_CONNECTION_ESTABLISHED, {
            userId: peer.userId,
            username: peer.username
    ...
    channel.onclose = () => {
      ...
      const peer = this.currentConnection?.peers[remoteUserId];
      if (peer) {
        this.sdk.notifyGame(WavedashEvents.P2P_PEER_DISCONNECTED, {
          userId: peer.userId,
          username: peer.username
